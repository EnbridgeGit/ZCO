REPORT  zbpci002_costcenterbudget MESSAGE-ID zs.
************************************************************************
*  Client:     Spectra Energy.                                         *
*  Date:       December 2010                                           *
*  Author:     Brian Boundy                                            *
*  Program Description:                                                *
*  This program will extract data for cost center budget for           *
*  the BPC application.                                                *
*                                                                      *
************************************************************************
*CHANGES:                                                              *
...
*2013/12/10 M Khan   SDP57444 Change logic for Category and file name  *
*                             depending upon the version #.            *
*2014/08/05 G Ymana  SDP58326 Remove activity type from COEP-OBJNR     *
*                    EAM      before populating Entity column          *
************************************************************************
TABLES: coej.
...
s_objnr   FOR coej-objnr OBLIGATORY
          DEFAULT 'KS00000000000000' TO 'KS9999999999999999'.
PARAMETERS:
...
PARAMETERS:
p_versn   LIKE coej-versn OBLIGATORY DEFAULT 0.     "SDP57444
*s_versn   FOR coej-versn OBLIGATORY DEFAULT 0.     "SDP57444
...
*           p_file    TYPE        string DEFAULT 'C:\SAPTEMP\CCBudget.csv', "TR995
            p_file    TYPE        string DEFAULT 'H:\SAPTEMP\CCBudget.csv', "TR995
...
START-OF-SELECTION.
  ...
  IF p_versn > '000'.
  CONCATENATE csvfile 'SAP-' p_gjahr '-CC-OMv'                   "SDP57444
                  p_versn '.csv' INTO csvfile.                   "SDP57444
  ELSE.                                                          "SDP57444
  CONCATENATE csvfile 'SAP-' p_gjahr '-CC-Bud.csv' INTO csvfile.
  ENDIF.                                                         "SDP57444
...
FORM get_db_data.
  ...
  SELECT objnr gjahr wrttp kstar versn
    ...
    WHERE objnr IN s_objnr
      ...
      AND versn EQ p_versn                                      "SDP57444
*      AND versn IN s_versn                                     "SDP57444
    .
...
*Set the hard-coded values.
  ...
  LOOP AT t_coej_sum INTO s_coej.
    ...
    IF p_versn = '000'.                                     "SDP57444
       CONCATENATE 'Budget' s_coej-gjahr INTO lv_category.
    ELSE.                                                   "SDP57444
       CONCATENATE 'OMv' p_versn INTO lv_category.          "SDP57444
    ENDIF.                                                  "SDP57444
    lv_entity  = s_coej-objnr+6(10).                        "SDP58326
    ...
    SELECT SINGLE bukrs
      INTO lv_bukrs
      FROM csks
      WHERE kostl = s_coej-objnr+6
      .

    SELECT SINGLE affil
      INTO lv_intco
      FROM zacctnew
      WHERE cocode  = lv_bukrs
