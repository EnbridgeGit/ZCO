REPORT ZKCOR001 MESSAGE-ID ZK LINE-SIZE 132 LINE-COUNT 65
...
* in that order.                                                   *
********************************************************************
* 2001/06/28 mdemeest 4.6B Fix pulldown for Cost Center Group
* 2001/02/17 mdemeest 4.6B Fix function calls
* 1998/12/08 modified by Nancy Gilligan, OmniLogic D30K906571
*            add cost element group defaults based on client &
*            show in header
* 1998/09/30 md7140 #--- Separation project
* 1997/06/04 Transport: D30K904106 - requested by J. Mieras
*            Add Internal Postings to report - COSS
*            Cost elements 5000, 6000 & 9000 series
********************************************************************
...
DATA:  ACTCST      LIKE COSP-WKG001,             "Actual Cost
       PLNCST      LIKE ACTCST,                  "Plan Cost
       ABSVAR      LIKE ACTCST,                  "Abs. var.
       VAR%(5)     TYPE P DECIMALS 2,            "Variance %
       ACTTOT(9)   TYPE P DECIMALS 2,            "Actual total
       PLNTOT      LIKE ACTTOT,                  "Plan total
       ABSTOT      LIKE ACTTOT,                  "Abs. var. total
       VARTOT      LIKE VAR%,                    "Variance total
       GRACT       LIKE ACTTOT,                  "Grand total actuals
       GRPLN       LIKE ACTTOT,                  "Grand total plan
       GRABS       LIKE ACTTOT,                  "Grand total abs. var.
       GRVAR       LIKE VAR%,                    "Grand total var. %
       LN_CNTR     TYPE I,                       "Line counter
       LTEXT       LIKE CSKT-LTEXT,              "Cost centre text
       GRPTEXT     LIKE CSKT-LTEXT,
       wa_setnr like T800y-setnr.
*       grptext(30) type c.                     "Cost centre Group txt
...
PARAMETERS: P_BUKRS    LIKE CSKS-BUKRS MODIF ID ABC,  "Company Code
            ...
            P_KTOPL    LIKE T800S-SEARCHFLD
                                  obligatory memory id kpl modif id abc
                                  default 'COAT'.    "Chart of Accounts
...
AT SELECTION-SCREEN ON VALUE-REQUEST FOR P_CTRGRP.
  ...
  CALL FUNCTION 'K_GROUP_SELECT'
       EXPORTING
            CLASS              = '0101'             "mdemeest 2001/02/19
            FIELD_NAME         = 'KOSTL'
...
DATA: O_TMPSET LIKE RGSB4-SETNR.
  ...
  DATA: O_SEARCHFLD(30) type c.
  o_searchfld(4)    = p_ktopl.
...
FORM GET_COSTGROUP.
data: setnr_func(30) type c.
data: searchfld_func like rgsbs-searchfld.
data: valtab like rgsbs-title.

  concatenate '0H' p_ctrgrp into wa_setnr.     "prior to 4.6B
  select single * from T800y
    ...
    if sy-subrc = '0'.
       ...
       select single * from setheadert          "=4.6B
          where setclass = '0101'
            and subclass = p_kokrs
            and setname  = p_ctrgrp
            and langu    = sy-langu.
        if sy-subrc = '0'.
           move setheadert-descript to grptext.
        endif.
    endif.


  move p_kokrs    to setnr_func.                    "mdemeest 2001/02/19
  move p_ctrgrp   to setnr_func+4(16).              "mdemeest 2001/02/19
  CALL FUNCTION 'G_SET_GET_ALL_VALUES'
       EXPORTING
            CLASS         = '0101'
            SETNR         = setnr_func              "mdemeest 2001/02/19
            TABLE         = 'CCSS'
       TABLES
            SET_VALUES    = SETVAL
       EXCEPTIONS
...
data:  setnr_func(30)    type c.                    "mdemeest 2001/02/19

  move p_ktopl        to setnr_func.                "mdemeest 2001/02/19
  move p_elmgrp       to setnr_func+4(20).          "mdemeest 2001/02/19
  CALL FUNCTION 'G_SET_GET_ALL_VALUES'
       EXPORTING
  ...
  LOOP AT ITAB1.
    ...
    IF ITAB2-KSTAR = ITAB1-KSTAR.
      SELECT SINGLE * FROM CSKU WHERE KSTAR = ITAB2-KSTAR
                                  ...
                                  AND SPRAS = SY-LANGU.
      PERFORM MOVE_ELM.
*      SELECT * FROM T800Y WHERE LANGU = SY-LANGU   "mdemeest 2001/02/19
...
*---------------------------------------------------------------------*
FORM PROC_MAINTAB.
SORT ITAB1 BY KOSTL ELMGRP KSTAR.
  LOOP AT ITAB1.
...
FORM WRT_CSTGRP.
  ...
  WRITE: /1 SY-VLINE, 6 ITAB1-KOSTL, 15 SY-VLINE,
         16(33) CSKT-LTEXT, 49 SY-VLINE, 71 SY-VLINE,
         93 SY-VLINE, 115 SY-VLINE, 132 SY-VLINE.
...
FORM CAL_COSTS.
  ADD ITAB1-ACOST1 FROM P_FROM TO P_TO GIVING ACTCST.
...
FORM WRT_DET.
  WRITE: /1 SY-VLINE, 15 SY-VLINE.
  FORMAT COLOR COL_KEY ON.
  WRITE:  21 ITAB1-KSTAR, 29 ITAB1-KTEXT.
  FORMAT COLOR COL_KEY OFF.
  FORMAT COLOR COL_NORMAL ON INTENSIFIED OFF.
  WRITE:  49 SY-VLINE, 71 SY-VLINE.
  IF ACTCST NE 0.
    WRITE:  50 ACTCST.
  ENDIF.
  IF PLNCST NE 0.
...
FORM WRT_ELMGRP.
  ...
  WRITE: 18 ITAB1-ELMGRP, 49 SY-VLINE.
  IF ACTCST NE 0.
    WRITE: 50 ACTCST.
  ENDIF.
  IF PLNCST NE 0.
...
FORM WRT_CSTOT.
  ...
  IF PLNTOT NE 0.
    WRITE: 74 PLNTOT.
  ENDIF.
  WRITE: 49 SY-VLINE,  71 SY-VLINE,  93 SY-VLINE.
  IF ABSTOT NE 0.
    WRITE: 96 ABSTOT.
  ENDIF.
  IF VARTOT NE 0.
...
FORM WRT_GRABS.
  ...
  FORMAT COLOR COL_TOTAL ON.
  ULINE: /1(132).
  WRITE: /1 SY-VLINE, 2 P_CTRGRP, 15 SY-VLINE,
         21 TEXT-016,49 SY-VLINE.
  IF GRACT NE 0.
    WRITE: 52 GRACT.
  ENDIF.
  IF GRPLN NE 0.
...
FORM WRT_HDG.
  ...
  WRITE: /1 SY-VLINE, 49 TEXT-TT3, 68 P_FROM, 72 TEXT-009, 76 P_TO,
          80 P_YEAR, 132 SY-VLINE.

  ULINE: /1(132).
  FORMAT COLOR COL_TOTAL ON INTENSIFIED OFF.
  WRITE: /1 SY-VLINE, 2 TEXT-006, 25 P_CTRGRP, 40 GRPTEXT,
            TEXT-025, P_ELMGRP(20), 132 SY-VLINE.
...
FORM WRT_COL.
  ULINE: /1(132).
  FORMAT INTENSIFIED OFF.
  WRITE: /1 SY-VLINE, 3 TEXT-007, 15 SY-VLINE, 16 TEXT-010, 49 SY-VLINE,
         56 TEXT-011, 71 SY-VLINE, 78 TEXT-012, 93 SY-VLINE,
        101 TEXT-013, 115 SY-VLINE, 121 TEXT-014, 132 SY-VLINE.
  ULINE: /1(132).
  LN_CNTR = LN_CNTR + 3.
  FORMAT INTENSIFIED ON.
ENDFORM.
