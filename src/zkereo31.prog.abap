...
***********************************************************************
*
* 09.08.99 4.X         Aufwärtskompatible Behandlung von REC_WAERS
*                      -> für 4.X die Vorlage rkevrk2e_4X.dnl
*                         verwenden.
* 22.12.98 Projekt     Hinzufügen Selektion der Vorgangsart
*                      dh981222
* 26.03.97 Experiment. Benachrichtigung über Fehler im Kasten ganz
*                      unten im Protokoll (sonstige RC's noch nicht
*                      eliminiert).
*                      Laufzeit / 100' Sätze ausgewiesen.
* 17.03.97             Fehlerkorrektur delete_ce3:
*                      Returncode DELETE ignorieren
* 13.03.97 22* 31H 40A Alternativentwicklung
*                      Scan über CE1, schreiben Datafile CE3
*                      Einschränkung: Temp-Platz für sortieren CE1
*                                     Max. Größe Datafile CE3 <=2GB
*                      Für Release 2.2 nur via Hinweis erhältlich.
*                      Release 2.1 wird nicht unterstützt.
* ---------------------------------------------------------------------
* 12.01.96     22H 30C Anzahl Ausgaben im Standard-Fall verringert
* 04.01.96     22H 30C Version für Auslieferung zusammengestellt
* ---------------------------------------------------------------------
* 14.07.95 21G 22F     Performance-Verbesserung für ORACLE
* ---------------------------------------------------------------------
* 19.05.95 21F 22E     viele PARAMETERS auf NO-DISPLAY gesetzt,
*                      damit der User nicht komplett verwirrt wird.
*                      Reorganisation auch für Plandaten eingebaut
* 18.05.95 21F         Portierung auf 2.1
* ---------------------------------------------------------------------
*    04.95     22C 30A Neuentwicklung aus Performancegründen
...
*   Selektionsbild
*
*   VERBOSE    ausführliche Protokollierung           RKEVRK2E
*   LOCKCE3    Verbuchersperre EKC3xxxx setzen        RKEREO31
*   SINGLDEL   CE3 mit einem Statement löschen        RKEVRK2E
*   GENONLY    nur Generieren                         RKEREO31
*   TESTMODE   Testmodus ohne Löschen/Schreiben CE3   RKEVRK2E
...
PARAMETERS:
  ...
  , RAMAVAIL     TYPE I DEFAULT 50 NO-DISPLAY     " nicht unterstützt
  .
SELECTION-SCREEN ULINE.

PARAMETERS: VERBOSE  DEFAULT 'X' NO-DISPLAY
          , LOCKCE3  DEFAULT 'X' NO-DISPLAY
          , SINGLDEL DEFAULT 'X' NO-DISPLAY       " nicht unterstützt
          , GENONLY  DEFAULT ' ' NO-DISPLAY
          , TESTMODE AS CHECKBOX DEFAULT ' '
*         nur während der Testzeit
...
************************************************************************
*
*   Ausgabefunktionen für den Parameter-Schirm
*
...
************************************************************************
*
*   Ausgabefunktionen für die Statistik-Rähmchen
...
* fester Teil: ERKRS, GJAHR, PAOBJNR, Zeit von...bis
  ...
  WRITE: /(60) SY-ULINE.
      WRITE: / SY-VLINE,
      3 'Anzahl gelöschte Objektebenensätze'(106),
      49 STAT-DEL_CE3,
      60 SY-VLINE.
  WRITE: / SY-VLINE,
