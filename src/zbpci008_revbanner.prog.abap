REPORT  zbpci008_revbanner MESSAGE-ID zs.
************************************************************************
...
*CHANGES:                                                              *
*Date       By       Description                                       *
*2012/08/20 M Khan   Change C: drive to H: drive with                  *
*TR995               directory, file selection using F4                *
*                                                                      *
*2013/12/17 G Ymana  Modified program to allow volume amounts to be    *
*SDP60315            displayed with 3 decimal points.                  *
************************************************************************
TABLES: ce11100.
...
TYPES:  BEGIN OF ty_output,
          ...
          projectst(12)   TYPE c,
          paths(7)        TYPE c,
          amount(15)      TYPE p DECIMALS 3,                "SDP60315
        END OF ty_output.

DATA: lv_local      TYPE integer,
      wa_ce11100    LIKE coep,
      s_ce11100     TYPE ce11100, "ty_ce11100,
      t_ce11100     LIKE TABLE OF ce11100,                  "s_ce11100,
      t_ce11100_sum LIKE TABLE OF ce11100,                  "s_ce11100,
...
PARAMETERS: p_local   RADIOBUTTON GROUP rad1 DEFAULT 'X',
            p_file    TYPE string
                      DEFAULT 'H:\SAPTEMP\REVBanner.csv', "TR995
...
*  SELECT paledger gjahr perde wwpce rbeln bukrs wwsub wwser
*         wwprg wwrat werks vvbvl vvbrv kndnr wwsld
...
*      AND wwpce     IN s_wwpce.
  ...
  SELECT * INTO CORRESPONDING FIELDS OF TABLE t_ce11100
      FROM ce11100
      WHERE paledger  = p_paledg
        AND vrgar     IN s_vrgar
        AND versi     = ''
...
*Last sorted field is wwrsn
    AT END OF wwrsn. "wwpce.
      SUM.
...
*Set the hard-coded values.
  ...
  LOOP AT t_ce11100_sum INTO s_ce11100.
    ...
    SELECT SINGLE blart INTO lv_blart
      FROM bkpf
      WHERE belnr = s_ce11100-rbeln
        AND gjahr = s_ce11100-gjahr
        AND bukrs = s_ce11100-bukrs.
    ...
    ENDCASE.
     ...
     IF s_ce11100-wwpce = 390590.
**        AND s_ce11100-kndnr = 'UNBILL'.
        IF s_ce11100-wwrsn = 'ZES'.
          lv_datasrc = 'SAP_Unbilled_Est'.
        ELSEif s_ce11100-wwrsn = 'ZRV'.
          lv_datasrc = 'SAP_Unbilled_Prior'.
        ELSE.
...
*Gas volume
    IF s_ce11100-vvbvl <> 0 AND
       ( s_ce11100-wwsub = 'DC' OR s_ce11100-wwsub = 'OD' OR
         s_Ce11100-wwsub = 'C1' OR s_ce11100-wwsub = 'C2' ).

      if s_ce11100-wwsub = 'DC' OR s_ce11100-wwsub = 'OD'.
         lv_account = 'GenServ_vol'.
      ELSEIF s_Ce11100-wwsub = 'C1'.
             lv_account = 'GenServ_vol_C1'.
      ELSEIF s_Ce11100-wwsub = 'C2'.
             lv_account = 'GenServ_vol_C2'.
      endif.
...
*      IF s_ce11100-wwseg = 'EX'.
*         s_output-projectst  = 'EXEMPT'.
*      ELSEIF s_ce11100-wwseg = 'NE'.
*        s_output-projectst  = 'NONEXEMPT'.
*      else.
*        s_output-projectst  = 'NOVAL'.
*      ENDIF.
...
*Add output table to the csvfile.
  LOOP AT t_output_sum INTO s_output.
    ...
    IF s_output-account = 'GenServ_vol'.                    "SDP60315
                                                            "SDP60315
